...
public class NBTProviderImpl implements NBTProvider{
    ...
    public NBTBase serializeUUID(UUID uuid){
        NBTTagCompound a = GameProfileSerializer.a(uuid);
        return fromNMS(a);
    }
    
    @Override
    public UUID deserializeUUID(NBTBase uuid){
        if(!(uuid instanceof NBTCompound)) return null;
        NBTTagCompound nbtTagCompound = (NBTTagCompound) toNMSTag(uuid);
        return GameProfileSerializer.b(nbtTagCompound);
    }
    
    @Override
    public ItemStack applyNBTToItem(ItemStack itemStack, NBTCompound tag){
        net.minecraft.server.v1_13_R2.ItemStack itemStack1 = CraftItemStack.asNMSCopy(itemStack);
    ...
    private net.minecraft.server.v1_13_R2.NBTBase toNMSTag(NBTBase tag){
    ...
    private NBTBase fromNMS(net.minecraft.server.v1_13_R2.NBTBase base){
        if(base==null)return null;
        switch(base.getTypeId()){
            case CraftMagicNumbers.NBT.TAG_BYTE:
                return new NBTByte(((net.minecraft.server.v1_13_R2.NBTNumber) base).asByte());
            case CraftMagicNumbers.NBT.TAG_SHORT:
                return new NBTShort(((net.minecraft.server.v1_13_R2.NBTNumber) base).asShort());
            case CraftMagicNumbers.NBT.TAG_INT:
                return new NBTInt(((net.minecraft.server.v1_13_R2.NBTNumber) base).asInt());
            case CraftMagicNumbers.NBT.TAG_LONG:
                return new NBTLong(((net.minecraft.server.v1_13_R2.NBTNumber) base).asLong());
            case CraftMagicNumbers.NBT.TAG_FLOAT:
                return new NBTFloat(((net.minecraft.server.v1_13_R2.NBTNumber) base).asFloat());
            case CraftMagicNumbers.NBT.TAG_DOUBLE:
                return new NBTDouble(((net.minecraft.server.v1_13_R2.NBTNumber) base).asDouble());
            case CraftMagicNumbers.NBT.TAG_STRING:
                return new NBTString(base.asString());
            case CraftMagicNumbers.NBT.TAG_BYTE_ARRAY:
                return new NBTListByte(((NBTTagByteArray) base).c());
            case CraftMagicNumbers.NBT.TAG_INT_ARRAY:
                return new NBTListInt(((NBTTagIntArray) base).d());
            case 12: //For some reason not listed in CraftMagicNumbers.NBT
                return new NBTListLong(((NBTTagLongArray) base).d());
            case CraftMagicNumbers.NBT.TAG_LIST:
                NBTTagList nbtList = (NBTTagList) base;
                NBTListTag nbtListTag = new NBTListTag(Collections.emptyList(), (byte) nbtList.d());
                for(net.minecraft.server.v1_13_R2.NBTBase nbtBase : nbtList){
